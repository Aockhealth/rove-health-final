"use client";

import { useEffect, useRef, useState } from "react";
import { cn } from "@/lib/utils";
import { useReducedMotion } from "@/lib/useReducedMotion";

export type MechanismStep = { scene: string; detail: string };

const STEPS: MechanismStep[] = [
  {
    scene: "Cells stop listening.",
    detail: "Cells respond less efficiently to insulin.",
  },
  {
    scene: "So the body sends more.",
    detail: "The pancreas compensates by producing more of it.",
  },
  {
    scene: "The ovaries answer.",
    detail: "That excess insulin drives the ovaries to overproduce androgens.",
  },
  {
    scene: "And ovulation loses the thread.",
    detail: "Disrupting ovulation.",
  },
  {
    scene: "Which is never just a late period.",
    detail: "The same androgens show up in your cycle, and on your skin.",
  },
];

export function MechanismChain({ steps = STEPS }: { steps?: MechanismStep[] }) {
  const stepRefs = useRef<(HTMLLIElement | null)[]>([]);
  const [lit, setLit] = useState<boolean[]>(() => steps.map(() => false));
  const reduced = useReducedMotion();

  useEffect(() => {
    if (reduced) return;

    // One observer per step rather than a scroll listener: a step lights up
    // once, when it crosses into the lower three-quarters of the viewport, and
    // never re-measures on subsequent frames.
    const observers = stepRefs.current.map((el, i) => {
      if (!el) return null;
      const observer = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            setLit((prev) => {
              if (prev[i]) return prev;
              const next = [...prev];
              next[i] = true;
              return next;
            });
            observer.disconnect();
          }
        },
        { rootMargin: "0px 0px -25% 0px" }
      );
      observer.observe(el);
      return observer;
    });

    return () => observers.forEach((observer) => observer?.disconnect());
  }, [reduced, steps.length]);

  const active = reduced ? steps.map(() => true) : lit;

  return (
    <ol className="relative">
      {/* The spine the steps hang off */}
      <span
        aria-hidden
        className="absolute left-[13px] top-2 bottom-2 w-px bg-phase-follicular/40 md:left-[15px]"
      />

      {steps.map((step, i) => {
        const isOn = active[i];
        return (
          <li
            key={step.scene}
            ref={(el) => {
              stepRefs.current[i] = el;
            }}
            className="relative flex gap-4 pb-6 last:pb-0"
          >
            <span
              aria-hidden
              className={cn(
                "relative z-10 mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border transition-all duration-500 md:h-8 md:w-8",
                isOn
                  ? "border-obsidian bg-obsidian scale-100"
                  : "border-obsidian/15 bg-white-bone scale-90"
              )}
            >
              <span
                className={cn(
                  "font-sans text-[11px] font-semibold transition-colors duration-500 md:text-xs",
                  isOn ? "text-white-bone" : "text-obsidian/70"
                )}
              >
                {i + 1}
              </span>
            </span>

            <div
              className="transition-all duration-700 ease-out"
              style={{
                opacity: isOn ? 1 : 0.35,
                transform: isOn ? "translateY(0)" : "translateY(6px)",
              }}
            >
              <p className="font-sans text-base font-semibold tracking-tight text-obsidian md:text-lg">
                {step.scene}
              </p>
              <p className="mt-0.5 font-sans text-sm leading-relaxed text-obsidian/65">
                {step.detail}
              </p>
            </div>
          </li>
        );
      })}
    </ol>
  );
}
