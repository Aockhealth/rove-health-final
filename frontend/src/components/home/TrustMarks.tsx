...
export function TrustMarks() {
  return (
    <section className="px-6 py-8">
      <Reveal className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-x-8 gap-y-4 rounded-[20px] border border-obsidian/8 bg-white px-6 py-5 shadow-[0_8px_24px_rgba(0,0,0,0.05)] md:px-10">
        {MARKS.map(({ icon: Icon, label }) => (
          <span key={label} className="flex items-center gap-2">
            <Icon className="h-4 w-4 shrink-0 text-obsidian/60" strokeWidth={1.75} aria-hidden />
