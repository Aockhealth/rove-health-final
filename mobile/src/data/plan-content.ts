/**
 * Mirrors the BLUEPRINTS inline constant in
 * frontend/src/app/cycle-sync/plan/page.tsx. Kept as a local copy (not moved to
 * shared/) because the web version isn't exported/reusable there either — same
 * treatment as mobile/src/data/home-content.ts.
 *
 * Icon fields that were lucide-react component references on web (diet.core_needs,
 * diet.ideal_meals) are string names here instead, since shared-shaped data can't
 * hold a web-only component reference. A direct grep of plan/page.tsx confirms
 * those two specific fields aren't actually rendered anywhere in the current UI
 * (only rituals.practices' string icons, hormones, nutrition_guide, and exercise
 * are rendered) — so this conversion changes no visible behavior.
 */

export type PlanBlueprint = {
  color: string;
  hormones: { title: string; summary: string; desc: string; symptoms: string[] };
  rituals: {
    focus: string;
    practices: { title: string; desc: string; icon: string }[];
    symptom_relief: { symptom: string; remedy: string }[];
  };
  diet: {
    core_needs: { id: string; title: string; desc: string; icon: string }[];
  ...
  exercise: {
    summary: string;
    best: { title: string; desc: string; time: string }[];
    avoid: string[];
  };
  ...
  nutrition_guide: {
    macro_fuel: {
      ...
      proteinLabel: string; fatsLabel: string; carbsLabel: string;
      proteinDesc: string; fatsDesc: string; carbsDesc: string;
    };
    symptom_decoder: {
      title: string;
      subtitle: string;
      cards: { title: string; condition: string; biology: string; diet: string }[];
    };
    cheat_sheet: {
...
export const BLUEPRINTS: Record<string, PlanBlueprint> = {
  Menstrual: {
