...
**Update 2026-07-20 — Flow Card gap closed, plus a real bug fix and design pass:** built `FlowCard.tsx` and wired the phase-based swap web does (`currentPhase === "Menstrual" ? FlowCard : DischargeCard`) — mobile previously always showed Discharge regardless of phase. Also fixed a real bug: the effect populating the daily-log form depended on the whole `monthLogs` object rather than the selected day's own entry, so any unrelated mutation (toggling a different day's period status, a background month fetch) silently wiped out unsaved edits — narrowed to depend on `selectedLog` instead, and hid the log cards + Save button during period-logging mode (matching web) since that's what made the collision easy to trigger. Design pass: chips were a full pill (`borderRadius: 50`) everywhere web uses `rounded-xl` (~12px) instead, chip rows were missing `alignItems: 'flex-start'` (RN's default `stretch` was matching every chip in a row to the tallest one), and every card's icon badge was a rounded-square where web's are `rounded-full` circles — all fixed. Added haptics (chip toggles, period-logging taps, save success) and a swipe gesture on the calendar to change months.
...
### [x] 22. Rove Chef — REBUILT as v2 "Pick Your Plate" 2026-07-22, deploy + phone checkpoint pending
**v2 rework (2026-07-22, founder-directed):** replaced the single-invented-recipe flow with a pick-from-4-real-dishes flow with per-user preference learning. Full design + deployment checklist in `docs/ai-chef-v2-redesign-plan.md`. New: `user_food_choices` migration (must be applied to live Supabase), `/api/rove-chef-options` + `/api/rove-chef-detail` routes (must be deployed to rovehealth.in), `mobile/src/lib/foodChoices.ts`, reworked `RoveChef.tsx`. The production-URL bug flagged below is FIXED (`EXPO_PUBLIC_API_URL` in `mobile/.env` + `api.ts` fallback to rovehealth.in) — this also un-breaks Item 23's Coach in production builds. Old v1 endpoint kept as fallback until the phone checkpoint passes.
**Original v1 notes (superseded):**
...
**Production-breaking bug found 2026-07-20, not yet fixed:** `RoveChef.tsx` calls the real `generateRoveChefProtocol()` in `mobile/src/lib/api.ts`, which is correctly wired end-to-end — but `api.ts`'s `getBaseUrl()` returns the literal placeholder string `'https://your-production-url.com'` for any non-`__DEV__` build. Every "Generate" tap in a real (TestFlight/Play Store/EAS preview) build will fetch a nonexistent domain and fail — caught by a try/catch so it won't crash, but shows the user an "Error: Could not generate recipe" alert every time. Same root cause affects Item 23 (Exercise Builder / Rove Coach) below — one shared fix. No `EXPO_PUBLIC_API_URL` (or equivalent) exists in `.env` today; needs one added, defaulting to `https://rovehealth.in`, and `getBaseUrl()` updated to read it.
...
**Same production-breaking bug as Item 22:** calls the real `generateRoveCoachPlan()`, but hits the same placeholder production URL in `mobile/src/lib/api.ts`. One fix (the `EXPO_PUBLIC_API_URL` env var) closes both items.
...
### [ ] 27. Splash screen, status bar, app icons, offline caching, push scaffolding — partially built (2026-07-19 audit)
...
**What's still missing, checked directly:** no `react-native-mmkv` dependency and no `persistQueryClient` anywhere — offline caching of learn content / last-known cycle data isn't built. No `expo-notifications` dependency — push scaffolding isn't started. This item is genuinely still open overall; only the icon/splash/status-bar third of it is done.
